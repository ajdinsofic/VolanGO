...
export class HomeComponent implements OnInit, OnDestroy{
  ...
  constructor(private authLoginService: AuthLoginEndpointService, private router: Router,
    private authService: MyAuthService, private route : ActivatedRoute, private cartService: CartService) {
  ...
  GenerateOffers(): void {
    this.router.navigateByUrl('/public/generateOffers', { state: { viewRequest: this.viewRequest } });
  }
  ...
  // Početna lokacija (možete promeniti)
    ...
    this.map = new google.maps.Map(document.querySelector("#map") as HTMLElement, mapOptions);
  ...
  isSameDateAndTime(pickUpDate: string, dropOffDate: string, pickUpTime: string, dropOffTime: string): boolean {
    ...
    // Upoređujemo vreme
    const pickUpTimeObj = parseInt(pickUpTime, 10);
    const dropOffTimeObj = parseInt(dropOffTime, 10);

    const isSameTime = pickUpTimeObj === dropOffTimeObj;
  ...
  submitForm(form: any, viewRequest: ViewRequest): void {
    ...
    if(new Date(viewRequest.PickUpDate) > new Date(viewRequest.DropOffDate)){
       console.log("Dates arent in the future")
       return;
    }
    else if (this.isSameDateAndTime(viewRequest.PickUpDate, viewRequest.DropOffDate, viewRequest.PickUpTime, viewRequest.DropOffTime)) {
      console.log("Date and time the same");
      return;
    }
    // Prvo proverite da li je forma validna
    if (form.valid) {
      ...
      this.router.navigate(['/public/generateOffers'], {queryParams: { data: jsonString }});  // Primer navigacije
    } else {
      console.log('Forma nije validna');
      // Ovde možete dodati poruku korisniku da popuni sva polja
    }
  }
